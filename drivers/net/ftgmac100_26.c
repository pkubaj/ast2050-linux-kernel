...
static int ftgmac_is_macaddr_param(uint8_t *dev_mac)
{
	int i, j, got_num, num;
	...
	if (j == MAC_ADDR_LEN) {
		DO_PRINT("Overriding MAC address with: "
...
no_phy_access:
	...
	if (priv->NCSI_support == 0) {
		...
		if (out->miiPhyId == 0x8201) { //Realtek PHY at address 0
			return;
		}
...
static void ftgmac100_reset( struct net_device* dev )
{
  ...
  if ((priv->NCSI_support == 1) || (priv->INTEL_NCSI_EVA_support == 1)) {
    ...
    if (0==(tmp & (1u<<5 | 1u<<2)) || ids->phyAddr >= 0xff) {
       ...
       ((ids->miiPhyId & PHYID_VENDOR_MODEL_MASK) == PHYID_RTL8201N)) {
...
//					Disable_Channel(dev);
	...
	if (((priv->ids.miiPhyId & PHYID_VENDOR_MASK) == PHYID_VENDOR_MARVELL) ||
		...
		outl(
		 ...
		 ((priv->ids.miiPhyId & PHYID_VENDOR_MODEL_MASK) == PHYID_RTL8201N)) {
		outl(
			AHB_ERR_bit			|
			TPKT_LOST_bit		|
			TPKT2E_bit			|
...
//			}
		...
		if (packet_length > 0)
		{
			struct sk_buff  * skb = priv->rx_skbuff[cur_idx];
			u8		* data = 0;
...
//	printk("irq %d, mapped at %x\n", netdev->irq, (u32)netdev->base_addr);
	...
	ftgmac_is_macaddr_param(netdev->dev_addr);
